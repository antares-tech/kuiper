...
serviceManager.init = function (appConfig) {
	return new Promise (function (resolve/*, reject*/) {
		...
		natsiface.open_channel (__channels.discovery, parseMsg);
		serviceManager.ask ();
...
serviceManager.ask = function () {
	...
	log.debug ({msg : msg, channel : __channels.pull}, 'sending msg on nats');
	natsiface.send (__channels.pull, msg);
};
...
serviceManager.getInstancesCached = function (s_name) {
	/*
	 * This returns cached maps optimised for load-balancer needs (array form)
...
function beaconActive (instance, reason) {
	log.debug ({instance, reason}, 'mark instance active');
	instance.failure_count = 0;
...
function addNamespace () {
	...
	__channels.discovery = `${__config.namespace}.${config.channels.discovery}`;
	__channels.pull      = `${__config.namespace}.${config.channels.pull}`;
}
