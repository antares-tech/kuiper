...
function main () {
	...
	try {
		...
		exec_stdout = execSync (`sudo bash -c 'cd /etc/nginx/sites-enabled; ln -sf ../sites-available/3apples default'`);
		...
		if (exec_stdout) {
			console.log (`!error : moving ${temp_conf_file} to "/etc/nginx/sites-available/3apples" failed`);
			console.log (`-----------`);
			console.log (`>>>> STDOUT`);
